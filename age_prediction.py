...
# we used 10 samples for validation and 21 samples for testing
...
from NN import AgePredictor

# Parse command-line arguments
def parse_args():
    parser = argparse.ArgumentParser(description="Age prediction model training script")
    ...
    parser.add_argument('--num_labeled', type=int, default=70, help='Number of labeled samples to use')
...
log_path = f'runs/age/linearprobing/pretrain_{pretrained}' if frozen else f'runs/age/{num_labeled}-shot/pretrain_{pretrained}'
...
for num, (subject, age) in enumerate(zip(df['Mindboggle101'], df['Age']), start=1):
    if age == '?':
        print(f'{subject} has no label, skipping...')
        continue
    ...
    data = Data(
        ...
        y=torch.tensor([[int(age)]]).to(torch.float32),
        name=subject
    )
    data = transform(data)
    dataset.append(data)
    print(num, subject, age, data)
...
model = AgePredictor(pretrained, frozen, hidden).to(device)
...
for epoch in range(epochs):
    loss, mae = model.train_step(device, train_loader)
    train_losses.append(loss)
    train_maes.append(mae)
    loss, mae = model.test_step(device, val_loader)
    val_losses.append(loss)
    val_maes.append(mae)
    if loss < val_losses[best_val_index]:
        best_val_index = epoch
        torch.save(model.state_dict(), f'{log_path}/age_model.pt')
    print(epoch, train_losses[-1], train_maes[-1], val_losses[-1], val_maes[-1])

# Load the best model and evaluate on the test set
model.load_state_dict(torch.load(f'{log_path}/age_model.pt'))
loss, mae = model.test_step(device, test_loader)
print(loss, mae)
